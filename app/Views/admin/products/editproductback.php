...
<form id="formAddAdderss" method="POST" action="<?php echo URL?>admin/product/update/<?=$rows->id?>"enctype="multipart/form-data">  
    ...
    <div class="container py-5">
        ...
        <option value="0"<?php if($rows->status == '0') { ?> selected="selected"<?php } ?>>none</option>
        <option value="1"<?php if($rows->status == '1') { ?> selected="selected"<?php } ?>>available</option>
        <option value="2"<?php if($rows->status == '2') { ?> selected="selected"<?php } ?>>not available</option>        
        </select>
        </div>
        ...
        <div class="col-12">
                <label>description *</label>
                    <br>
                <div class="form-group label-floating is-empty">
                    <textarea name="description" columns = "5" rows = "5" class="form-control summernote" ><?= $rows->description ?></textarea>
                </div>
            </div>
            <div class="col-md-4 col-xs-12">
        <label>order *</label>
